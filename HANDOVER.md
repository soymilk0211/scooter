...
路線圖**第 1、2、3 項已完成並在模擬器上實測**，第 4、5 項還沒動。
程式與文件都已 commit 並推上 `origin/main`（最新 `4f5c193`），沒有未完成的修改。

**57 項單元測試全綠**（core-rules 31、app 26），`./gradlew build` 通過，APK 53 MB。

**接手的人第一件事**：使用者正在用影像判讀工具判資料，**目前判了 0 筆**。
80 頁廊道、117 個路口方向（108 個有街景）已經產好，但**產出物在 `pipeline/build/`，
那個目錄是 gitignore 的** —— 換一台機器要重跑：

```bash
cd pipeline
python make_corridor_page.py --all     # 重產 80 頁，會重抓影像，約 15 分鐘
python make_corridor_page.py --serve   # 開成本機網站，開著別關
```

`build/cache/` 與 `build/corridor_images/` 也都在 gitignore 裡，所以換機器等於整批重抓。
`.env` 的 `MAPILLARY_TOKEN` 同樣沒有進版控，要另外帶。

**下一項是路線圖第 4 項**（各縣市結構 + §99 法源推導的修正）。使用者說「等判得
差不多了再說」，所以動之前先問。

**唯一還髒著的東西**：`pipeline/__pycache__/*.pyc` 被 git 追蹤，每跑一次管線
`git status` 就髒一次。已經開了另一個工作階段去處理（加進 `.gitignore` 再
`git rm -r --cached`），如果那邊沒做完就自己補掉。
...
**目前進度：0 筆。** 80 頁都還沒判過，`field_checks.json` 裡還沒有 `image_checks`
這個陣列 —— 第一次跑 `apply_image_checks.py` 時它才會被建出來。
...
| 4 | **各縣市結構**　← **下一項** | `TAIPEI_DEFAULTS` 改成以縣市為鍵的登錄表；**修正 §99 法源推導**（見下）；**臺/台 正規化**；`districts.py` 的 bbox 參數化 |
| 5 | **導航** | 地址／POI（離線、分縣市）、路線、Google 分享連結帶入目的地、分縣市離線包。最大一包 |

第 4 項已經有兩塊先做掉了，動手前先看一眼免得重工：

- **臺／台正規化**在 `make_corridor_page.canonical()` 與 `import_enforcement` 的
  快速道路線號比對裡各有一份。第 4 項要做的是把它收成一份共用的，而不是從頭寫。
- **固定測速已經是全國資料**（1,523 筆），只是 `SEED_CITIES` 目前鎖在臺北市。
  縣市登錄表一出來，那裡放寬即可。
...
1. **區間測速** —— 使用者明說**上線前一定要加回去**。schema 位子已經留好
   ...
   **原始資料每筆只有一組座標，但地址裡寫著頭尾里程**（例：`台9線68k+600m至56k+600m`、
   ...
   要從里程推 —— 那是實作時的第一道題，而且 44 筆裡臺北市只有 2 筆，其餘都在外縣市，
   等第 4 項的縣市結構做完才有意義。
