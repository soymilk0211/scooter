...
object EnforcementThresholds {
     ...
     *
     * 轉向指示是剛性的：太早講騎士會忘，太晚講來不及轉，所以它以**時間**計
     * （20 秒，見 [AlertThresholds.LEAD_SECONDS]）。測速不是 —— 五百公尺前講與
     * 兩百公尺前講同樣有效，因為減速隨時做得到，所以它留在距離上。
     * 窗開得比轉向指示遠，兩者就會自然錯開：測速在 500–380 公尺之間講完，
     * 轉向指示的窗（最遠 350 公尺）打開時語音通道已經空了。
     *
     ...
     *
     * **這個數字綁在剛性窗的上限上，不是獨立挑的。** 剛性窗改成時間計之後
     * （20 秒，上限 350 公尺），舊的 320 公尺會在時速 60 以上與它重疊 ——
     * 那正是兩則警示搶同一條語音通道的情況。留 30 公尺的餘裕，
     * `EnforcementMatcherTest` 有一條測試在守這個不變式。
     */
    const val MIN_DISTANCE_METERS = AlertThresholds.MAX_LEAD_METERS + 30.0
